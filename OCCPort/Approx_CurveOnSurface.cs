...
namespace OCCPort
{
    ...
    public class Approx_CurveOnSurface
    {
        public bool buildC3dOnIsoLine(Adaptor2d_Curve2d theC2D,
                                                           bool theIsU,
                                                           double theParam,
                                                           bool theIsForward)
        {
            // Convert adapter to the appropriate type.
            GeomAdaptor_Surface aGeomAdapter = mySurf as GeomAdaptor_Surface;
            if (aGeomAdapter == null)
                return false;

            if (mySurf._GetType() == GeomAbs_SurfaceType.GeomAbs_Sphere)
                return false;

            // Extract isoline
            Geom_Surface aSurf = aGeomAdapter.Surface();
            Geom_Curve aC3d;
            ...
            //bool isToTrim = true;
            //double U1, U2, V1, V2;
            //aSurf.Bounds(out U1, out U2, out V1, out V2);

            //if (theIsU)
            //{
            //    double aV1Param = Math.Min(aF2d.Y(), aL2d.Y());
            //    double aV2Param = Math.Max(aF2d.Y(), aL2d.Y());
            ...
            //    {
            //        if (Math.Abs(aV2Param - aV1Param) < Precision.PConfusion())
            //        {
            //            return false;
            //        }
            ...
            //    else
            //    {
            //        aV1Param = Math.Max(aV1Param, V1);
            //        aV2Param = Math.Min(aV2Param, V2);
            //        if (Math.Abs(aV2Param - aV1Param) < Precision.PConfusion())
            //        {
            //            return false;
            //        }
            ...
            //else
            //{
            //    double aU1Param = Math.Min(aF2d.X(), aL2d.X());
            //    double aU2Param = Math.Max(aF2d.X(), aL2d.X());
            ...
            //    else
            //    {
            //        aU1Param = Math. Max(aU1Param, U1);
            //        aU2Param = Math.Min(aU2Param, U2);
            //        if (Math.Abs(aU2Param - aU1Param) < Precision.PConfusion())
            //        {
            //            return false;
            //        }
            //    }
            ...
            //double aParF = myFirst;
            //double aParL = myLast;
            //int aNbPnt = 23;
            //for (int anIdx = 0; anIdx <= aNbPnt; ++anIdx)
            //{
            //    double aPar = aParF + ((aParL - aParF) * anIdx) / aNbPnt;

            //    gp_Pnt2d aPnt2d = theC2D.Value(aPar);

            //    gp_Pnt aPntC3D = myCurve3d.Value(aPar);
            //    gp_Pnt aPntC2D = mySurf.Value(aPnt2d.X(), aPnt2d.Y());

            //    double aSqDeviation = aPntC3D.SquareDistance(aPntC2D);
            //    myError3d = Math.Max(aSqDeviation, myError3d);
            //}

            //myError3d = Math.Sqrt(myError3d);
        ...
        public void Perform(int theMaxSegments,
                                     int theMaxDegree,
                                     ...
                                     bool theOnly2d)
        {
            ...
            if (theOnly3d && theOnly2d) throw new Standard_ConstructionError();

            GeomAbs_Shape aContinuity = theContinuity;
            if (aContinuity == GeomAbs_Shape.GeomAbs_G1)
                aContinuity = GeomAbs_Shape.GeomAbs_C1;
            else if (aContinuity == GeomAbs_Shape.GeomAbs_G2)
                aContinuity = GeomAbs_Shape.GeomAbs_C2;
            else if (aContinuity > GeomAbs_Shape.GeomAbs_C2)
                aContinuity = GeomAbs_Shape.GeomAbs_C2; //Restriction of AdvApprox_ApproxAFunction

            Adaptor2d_Curve2d TrimmedC2D = myC2D.Trim(myFirst, myLast, Precision.PConfusion());

            bool isU = false, isForward = false;//??
            double aParam = 0.0;//??
            ...
            //Adaptor3d_CurveOnSurface HCOnS = new Adaptor3d_CurveOnSurface(TrimmedC2D, mySurf);

            //int Num1DSS = 0, Num2DSS = 0, Num3DSS = 0;
            //TColStd_HArray1OfReal OneDTol;
            //TColStd_HArray1OfReal TwoDTolNul;
            //TColStd_HArray1OfReal ThreeDTol;

            //// create evaluators and choose appropriate one
            //Approx_CurveOnSurface_Eval3d Eval3dCvOnSurf = new Approx_CurveOnSurface_Eval3d(HCOnS, myFirst, myLast);
            ...
            //    if (!theOnly2d)
            //    {
            //        TColgp_Array1OfPnt Poles = new TColgp_Array1OfPnt(1, aApprox.NbPoles());
        ...
        public bool isIsoLine(Adaptor2d_Curve2d theC2D,
                                                  bool theIsU,
                                                  double theParam,
                                                  bool theIsForward)
        {
            ...
            gp_Pnt2d aLoc2d = new gp_Pnt2d();
            gp_Dir2d aDir2d = null;

            // Test type.
            GeomAbs_CurveType aType = theC2D._GetType();
            ...
            //{
            //    Geom2d_BSplineCurve aBSpline2d = theC2D.BSpline();
            ...
            //    gp_Vec2d aVec2d=new gp_Vec2d (aBSpline2d.Pole(1), aBSpline2d.Pole(2));
            ...
            //{
            //    Geom2d_BezierCurve aBezier2d = theC2D->Bezier();
        ...
        Geom2d_BSplineCurve myCurve2d;
        Geom_BSplineCurve myCurve3d;
        bool myIsDone;
        bool myHasResult;
        double myError3d;
        double myError2dU;
        double myError2dV;
        //! Input curve.
        Adaptor2d_Curve2d myC2D;

        //! Input surface.
        Adaptor3d_Surface mySurf;
        ...
        public Approx_CurveOnSurface(Adaptor2d_Curve2d theC2D,
            Adaptor3d_Surface theSurf,
            double theFirst, double theLast, double theTol)
        {
            myC2D = (theC2D);
            mySurf = (theSurf);
            myFirst = (theFirst);
